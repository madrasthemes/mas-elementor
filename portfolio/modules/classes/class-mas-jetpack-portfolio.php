	...
	class MAS_Jetpack_Portfolio {
		...
		function setting_html() {
			if ( current_theme_supports( self::CUSTOM_POST_TYPE ) ) : ?>
				<p><?php printf( /* translators: %s is the name of a custom post type such as "jetpack-portfolio" */ __( 'Your theme supports <strong>%s</strong>', 'mas-elementor' ), self::CUSTOM_POST_TYPE ); ?></p>
			<?php else : ?>
			...
			if ( get_option( self::OPTION_NAME, '0' ) || current_theme_supports( self::CUSTOM_POST_TYPE ) ) :
				printf(
					...
					sprintf(
						__( 'Portfolio pages display at most %1$s projects', 'mas-elementor' ),
						sprintf(
		...
		function updated_messages( $messages ) {
			...
			$messages[ self::CUSTOM_POST_TYPE ] = array(
				...
				1  => sprintf( __( 'Project updated. <a href="%s">View item</a>', 'mas-elementor' ), esc_url( get_permalink( $post->ID ) ) ),
				...
				5  => isset( $_GET['revision'] ) ? sprintf( esc_html__( 'Project restored to revision from %s', 'mas-elementor' ), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
				6  => sprintf( __( 'Project published. <a href="%s">View project</a>', 'mas-elementor' ), esc_url( get_permalink( $post->ID ) ) ),
				7  => esc_html__( 'Project saved.', 'mas-elementor' ),
				8  => sprintf( __( 'Project submitted. <a target="_blank" href="%s">Preview project</a>', 'mas-elementor' ), esc_url( add_query_arg( 'preview', 'true', get_permalink( $post->ID ) ) ) ),
				9  => sprintf(
					__( 'Project scheduled for: <strong>%1$s</strong>. <a target="_blank" href="%2$s">Preview project</a>', 'mas-elementor' ),
					// translators: Publish box date format, see https://php.net/date
					date_i18n( __( 'M j, Y @ G:i', 'mas-elementor' ), strtotime( $post->post_date ) ),
					esc_url( get_permalink( $post->ID ) )
				),
				10 => sprintf( __( 'Project item draft updated. <a target="_blank" href="%s">Preview project</a>', 'mas-elementor' ), esc_url( add_query_arg( 'preview', 'true', get_permalink( $post->ID ) ) ) ),
			);
		...
		static function portfolio_shortcode_html( $atts ) {
			...
			if ( $query->have_posts() ) {
				...
				?>
				<p><em><?php _e( 'Your Portfolio Archive currently has no entries. You can start creating them on your dashboard.', 'mas-elementor' ); ?></p></em>
				<?php
			}
			$html = ob_get_clean();
		...
		static function get_project_author() {
			...
			$html .= sprintf(
				__( '<span>Author:</span> <a href="%1$s">%2$s</a>', 'mas-elementor' ),
