...
class Skin_Style_2 extends Skin_Base {
    ...
    public function get_id() {
        return 'jobs-filter-skin-2';
    }

    public function get_title() {
        return esc_html__( 'Style 2', 'mas-elementor' );
    }
    ...
    public function render(){
        ...
        <form method="GET" action="<?php echo esc_url( $jobs_page_url ); ?>" class="mas-search-form"style="display:flex;">
			...
			<div class="mas-job-search-location">
				...
				<input type="text" id="search_location" name="search_location" placeholder="<?php echo esc_html( $settings['location_placeholder'] ); ?>" class="pac-target-input" autocomplete="off">
			</div>
			<div class="mas-job-search-category">
                ...
                <select id="search_category" name="search_category">
                    <option value=""><?php echo esc_html( 'cat' ); ?></option>
                    <?php foreach ( get_job_listing_categories() as $cat ) : ?>
                    <option value="<?php echo esc_attr( $cat->term_id ); ?>"><?php echo esc_html( $cat->name ); ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
			<div class="mas-job-search-submit">
				<button type="submit" value="Search"><i class="la la-search"></i><span class="mas-job-search-text"><?php echo esc_html( $settings['search_text'] ); ?></span></button>
			</div>
